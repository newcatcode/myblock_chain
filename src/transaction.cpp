...
Transaction::Transaction(std::string sender_key, std::string recipient_key, double amount, time_point timestamp)
    : _sender_key(std::move(sender_key)), _recipient_key(std::move(recipient_key)), _amount(amount), _timestamp(timestamp) {}
...
std::string Transaction::compute_hash() const{
    std::stringstream ss;
    ss << _sender_key << _recipient_key << _amount << std::chrono::duration_cast<std::chrono::milliseconds>(_timestamp.time_since_epoch()).count();
...
std::string Transaction::to_string() const {
    std::stringstream ss;
    ss << "Transaction from " << _sender_key << " to " << _recipient_key << " of amount " << _amount << " at time " << std::chrono::duration_cast
